    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            processing_options = st.multiselect(
                ...
                ["Denoise Image", "CLAHE", "Histogram Equalization","Canny Edge Detection", "Adaptive Thresholding","Morphological Operations"]
            )
            ...
            if processing_options:
                ...
                if "Morphological Operations" in processing_options:
...
def adaptive_thresholding(image):
    ...
    if len(image.shape) == 2:  # Grayscale image
        ...
        raise ValueError("Invalid image format. Expected a 2D (grayscale) or 3D (BGR) image.")
...
def histogram_equalization(image):
    ...
    return cv2.cvtColor(equalized, cv2.COLOR_GRAY2BGR)  # Convert back to BGR for output  # Convert back to BGR
